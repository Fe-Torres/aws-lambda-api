...
export class SimpleAnalytics implements IWebsiteAccess {
  ...
  async countWebsiteAccess(urlToAnalyse: string, params?: IAnalyticsParams): Promise<IWebsiteResponse> {
    try {
      ...
      const websiteResponse = analyticsRequestBuilder.buildWebsiteResponse(response.data);
